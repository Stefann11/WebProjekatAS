 ...
 * 
 * @Override public Role deserialize(JsonParser jsonParser,
 * DeserializationContext deserializationContext) throws IOException,
 * JsonProcessingException { JsonNode node =
 * jsonParser.getCodec().readTree(jsonParser); String role =
 * node.get("role").asText(); return Stream.of(Role.values()) .filter(enumValue
 * -> enumValue.getRole().equals(role)) .findFirst() .orElseThrow(() -> new
 * IllegalArgumentException("gender "+role+" is not recognized")); } }
 */
